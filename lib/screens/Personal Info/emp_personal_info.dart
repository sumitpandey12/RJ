...
import '../../models/referedByModel.dart';
import '../../utils/globals.dart';

class EmpPersonalInfoPage extends StatefulWidget {
  final EmployerModel? employer;

  const EmpPersonalInfoPage({super.key, required this.employer});
  @override
  _EmpPersonalInfoPageState createState() => _EmpPersonalInfoPageState();
}

class _EmpPersonalInfoPageState extends State<EmpPersonalInfoPage> {
  ...
  List<ReferralModel> _referredbyoptions = [];

  final List<String> _businesscategoryoptions = [
    isEnglishSelected ? 'Garments' : 'कपड़ा उद्योग',
    isEnglishSelected ? 'Bakery' : 'बेकरी',
    isEnglishSelected ? 'Hotel' : 'होटल',
    isEnglishSelected ? 'Hospitals' : 'अस्पताल',
    isEnglishSelected ? 'Paint' : 'पेंट',
    isEnglishSelected ? 'Automobile' : 'ऑटोमोबाइल',
    isEnglishSelected ? 'Glass' : 'शीशा',
    isEnglishSelected ? 'Medicines' : 'दवाई',
    isEnglishSelected ? 'Teaching' : 'शिक्षा',
    isEnglishSelected ? 'Beauty Salon' : 'ब्यूटी पार्लर',
    isEnglishSelected ? 'Furniture' : 'फ़र्नीचर',
    isEnglishSelected ? 'Construction' : 'निर्माण',
    isEnglishSelected ? 'Restaurants' : 'रेस्टोरेंट',
    isEnglishSelected ? 'Tailoring' : 'दर्जी',
    isEnglishSelected ? 'Hardware' : 'हार्डवेयर',
    isEnglishSelected ? 'Sanitary' : 'सैनिटरी',
    isEnglishSelected ? 'Factory' : 'फैक्टरी',
    isEnglishSelected ? 'Handloom' : 'हैंडलूम',
    isEnglishSelected ? 'Toys' : 'खिलौने',
    isEnglishSelected ? 'Decoration' : 'सजावट',
    isEnglishSelected ? 'Utencils' : 'बर्तन',
    isEnglishSelected ? 'Others' : 'अन्य',
  ];

  bool isProfileCreated = false;

  late TextEditingController _orgnizationNameController;
  ...
  void initState() {
    super.initState();
    fetchReferredBy();
    _orgnizationNameController = TextEditingController();
    _emailController = TextEditingController();
    _addressController = TextEditingController();

    if (widget.employer != null) {
      isProfileCreated = true;
      _orgnizationNameController.text = widget.employer!.organization ?? "";
      _addressController.text = widget.employer!.address ?? "";
      _emailController.text = widget.employer!.emailID ?? "";
      _state = widget.employer!.stateUt ?? "";
      _city = widget.employer!.city ?? "";
      _referredby = widget.employer!.referredBy ?? "";
      _businesscategory = widget.employer!.organizationType ?? "";
    }
  }

  void fetchReferredBy() async {
    final response = await http.get(Uri.parse(APIEndPoint.referedBy));
    ...
    if (response.statusCode == 200) {
      ...
      setState(() {
        _referredbyoptions =
            jsonResponse.map((e) => ReferralModel.fromJson(e)).toList();
      });
      log(_referredbyoptions.length.toString());
    } else {
                ...
                children: [
                  TextField(
                    decoration: InputDecoration(
                      labelText:
                          _isEnglishSelected ? "Organization Name" : "संगठन",
                      border: const UnderlineInputBorder(),
                      labelStyle: const TextStyle(
                        color: Color(0xFF0098DA),
                      ),
                    ),
                    controller: _orgnizationNameController,
                    onChanged: (value) {
                  ...
                  TextField(
                    decoration: InputDecoration(
                      labelText: _isEnglishSelected ? "Address" : "पता",
                      border: const UnderlineInputBorder(),
                      labelStyle: const TextStyle(
                        color: Color(0xFF0098DA),
                      ),
                  ...
                  DropdownButtonFormField<String>(
                    ...
                    onChanged: (value) {
                      setState(() {
                        _state = value;
                        _city = null; // reset option 2 when option 1 changes
                      });
                    },
                  ),
                  ...
                  DropdownButtonFormField<String>(
                    ...
                    items: _referredbyoptions.map((option) {
                      return DropdownMenuItem(
                        value: option.referrerName,
                        child: Text("${option.referrerName}"),
                      );
                    }).toList(),
                  ),
                  DropdownButtonFormField<String>(
                    ...
                    items: _businesscategoryoptions.map((option) {
                      return DropdownMenuItem(
                        value: option,
                        child: Text(option),
                      );
                  ...
                  GestureDetector(
                    onTap: () {
                      if (organizationName.isEmpty ||
                        ...
                        Utils.showSnackBar(context, "Fill all details first");
                        return;
                      }
                      EmployerService.postPersonalDetails(
                          context,
                          organizationName,
                          _city ?? "",
                          _state ?? "",
